...
Agile/Scrum"""
    ...
    # def resume_builder_page(self):
    #     """Resume builder page for converting uploaded resumes to company template"""
    #     st.markdown('<h2 class="main-header">📝 Resume Builder</h2>', unsafe_allow_html=True)
    #     
    #     # Check if resume builder is available
    #     if not RESUME_BUILDER_AVAILABLE:
    #         st.error("❌ Resume Builder functionality is not available. Please ensure resume_builder.py and required dependencies are installed.")
    #         st.info("Run: pip install -r requirements.txt to install missing dependencies.")
    #         return
    #     
    #     # Check PDF dependencies
    #     try:
    #         pdf_available, missing_deps = check_pdf_dependencies()
    #         if not pdf_available:
    #             st.error(f"❌ Missing PDF processing libraries: {', '.join(missing_deps)}")
    #             st.info("Run: pip install -r requirements.txt to install missing dependencies.")
    #             return
    #         
    #         # Initialize resume builder
    #         resume_builder = get_resume_builder(GEMINI_API_KEY)
    #         st.success("✅ Resume Builder initialized successfully")
    #         
    #     except Exception as e:
    #         st.error(f"❌ Failed to initialize Resume Builder: {e}")
    #         return
    #     
    #     # Create tabs for different functionalities
    #     tab1, tab2, tab3 = st.tabs(["📤 Upload & Convert", "📂 Manage Generated Resumes", "🎨 Template Info"])
    #     
    #     with tab1:
    #         self.resume_upload_interface(resume_builder)
    #     
    #     with tab2:
    #         self.resume_management_interface(resume_builder)
    #     
    #     with tab3:
    #         self.resume_template_info()
    
    # def resume_upload_interface(self, resume_builder):
    #     """Interface for uploading and converting resumes"""
    #     st.subheader("📤 Upload Resume & Convert to Template")
    #     
    #     # Information about the process
    #     st.markdown("""
    #     <div class="info-box">
    #     💡 <strong>How it works:</strong><br/>
    #     1. Upload your resume in PDF or DOCX format<br/>
    #     2. The system extracts information using AI parsing<br/>
    #     3. Your data is formatted according to our company template<br/>
    #     4. Download the newly generated professional resume
    #     </div>
    #     """, unsafe_allow_html=True)
    #     
    #     # File upload section
    ...
    #     
    #     supported_formats = resume_builder.get_supported_formats()
    ...
    #     )
        ...
        if uploaded_file is not None:
            ...
            with col3:
                st.metric("🗂️ File Type", uploaded_file.type)
            ...
            if st.button("🔄 Process Resume", type="primary"):
                try:
                    with st.spinner("📝 Extracting and parsing resume data..."):
                        ...
                        # Process the resume
                        parsed_data = resume_builder.process_resume(file_content, filename)
                    ...
                    with st.expander("👁️ Review Extracted Data", expanded=True):
                        col1, col2 = st.columns(2)
                        
                        with col1:
                            st.write("**Personal Information:**")
                            st.write(f"• **Name:** {parsed_data.get('full_name', 'Not found')}")
                            ...
                            if parsed_data.get('current_company'):
                                st.write(f"• **Company:** {parsed_data['current_company']}")
                            if parsed_data.get('current_position'):
                                st.write(f"• **Position:** {parsed_data['current_position']}")
                    ...
                    st.subheader("✏️ Edit Information (Optional)")
                    
                    with st.form("edit_resume_data"):
                        col1, col2 = st.columns(2)
                        
                        with col1:
                            edited_name = st.text_input(
                                "Full Name",
                                value=parsed_data.get('full_name', ''),
                                help="Edit if the name was not extracted correctly"
                            )
                            
                            edited_email = st.text_input(
                                "Email Address",
                                value=parsed_data.get('email', ''),
                                help="Edit if the email was not extracted correctly"
                            )
                            
                            edited_phone = st.text_input(
                                "Phone Number",
                                value=parsed_data.get('phone', ''),
                                help="Edit if the phone was not extracted correctly"
                            )
                        
                        with col2:
                            edited_company = st.text_input(
                                "Current Company",
                                value=parsed_data.get('current_company', ''),
                                help="Edit current company information"
                            )
                            
                            edited_position = st.text_input(
                                "Current Position",
                                value=parsed_data.get('current_position', ''),
                                help="Edit current position information"
                            )
                            ...
                            edited_experience = st.number_input(
                                ...
                                min_value=0,
                                max_value=50,
                                value=parsed_data.get('experience_years', 0) or 0,
                                help="Edit years of experience"
                            )
                        ...
                        edited_skills = st.text_area(
                            ...
                            value=skills_text,
                            height=100,
                            help="Edit or add skills, separate with commas"
                        )
                        ...
                        edited_summary = st.text_area(
                            ...
                            height=100,
                            help="Edit or add professional summary"
                        )
                        
                        # Generate resume button
                        if st.form_submit_button("🎨 Generate Resume PDF", type="primary"):
                            # Update parsed data with edits
                            updated_data = parsed_data.copy()
                            updated_data.update({
                                'full_name': edited_name.strip() if edited_name.strip() else parsed_data.get('full_name'),
                                'email': edited_email.strip() if edited_email.strip() else parsed_data.get('email'),
                                'phone': edited_phone.strip() if edited_phone.strip() else parsed_data.get('phone'),
                                'current_company': edited_company.strip() if edited_company.strip() else parsed_data.get('current_company'),
                                'current_position': edited_position.strip() if edited_position.strip() else parsed_data.get('current_position'),
                                'experience_years': edited_experience if edited_experience > 0 else parsed_data.get('experience_years'),
                                'skills': [s.strip() for s in edited_skills.split(',') if s.strip()] if edited_skills.strip() else parsed_data.get('skills'),
                                'summary': edited_summary.strip() if edited_summary.strip() else parsed_data.get('summary')
                            })
                            
                            try:
                                with st.spinner("🎨 Generating PDF resume..."):
                                    # Generate the PDF
                                    pdf_bytes, generated_filename = resume_builder.generate_resume_pdf(
                                        updated_data,
                                        updated_data.get('full_name')
                                    )
                                
                                st.success(f"✅ Resume generated successfully: {generated_filename}")
                                ...
                                with st.expander("📊 Generation Details", expanded=False):
                                    ...
                                    st.write(f"**Generated File:** {generated_filename}")
                                    st.write(f"**Template Used:** Batpharma Template")
                                    st.write(f"**Generated On:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                                    st.write(f"**File Size:** {len(pdf_bytes) / 1024:.1f} KB")
                    ...
                    st.info("💡 Please check that your file is a valid PDF or DOCX document and try again.")
    
    # def resume_management_interface(self, resume_builder):
    #     """Interface for managing generated resumes"""
    #     st.subheader("📂 Generated Resumes Management")
    #     
    #     # Get list of generated resumes
    #     generated_resumes = resume_builder.get_generated_resumes()
        
        if not generated_resumes:
            st.info("📝 No generated resumes found. Upload and convert a resume first.")
            return
        
        st.info(f"📊 Found {len(generated_resumes)} generated resume(s)")
        
        # Display generated resumes
        for i, resume_info in enumerate(generated_resumes):
            with st.expander(f"📄 {resume_info['filename']}", expanded=False):
                col1, col2, col3 = st.columns(3)
                
                with col1:
                    st.write(f"**📄 File:** {resume_info['filename']}")
                    st.write(f"**📏 Size:** {resume_info['size'] / 1024:.1f} KB")
                
                with col2:
                    st.write(f"**📅 Created:** {resume_info['created'][:16]}")
                    st.write(f"**🔄 Modified:** {resume_info['modified'][:16]}")
                
                with col3:
                    # Download button
                    resume_data = resume_builder.get_resume_data_from_file(resume_info['filename'])
                    if resume_data:
                        st.download_button(
                            label="📥 Download",
                            data=resume_data,
                    ...
                    if st.button(f"🗑️ Delete", key=f"delete_{i}"):
                        if st.session_state.get(f"confirm_delete_resume_{i}", False):
                            if resume_builder.delete_resume(resume_info['filename']):
                                st.success(f"✅ Deleted {resume_info['filename']}")
                                st.rerun()
                            else:
                                st.error(f"❌ Failed to delete {resume_info['filename']}")
                        else:
                            st.session_state[f"confirm_delete_resume_{i}"] = True
                            st.warning("⚠️ Click again to confirm deletion")
        ...
        if len(generated_resumes) > 1:
            ...
            with col1:
                if st.button("📦 Download All as ZIP"):
                    st.info("💡 ZIP download functionality can be added in future updates.")
            
            with col2:
                if st.button("🗑️ Clear All Generated Resumes"):
                    if st.session_state.get("confirm_clear_all_resumes", False):
                        deleted_count = 0
                        for resume_info in generated_resumes:
                            if resume_builder.delete_resume(resume_info['filename']):
                                deleted_count += 1
                        
                        st.success(f"✅ Deleted {deleted_count} resume(s)")
                        st.rerun()
                    else:
                        st.session_state["confirm_clear_all_resumes"] = True
                        st.warning("⚠️ Click again to confirm clearing all resumes")
    
    # def resume_template_info(self):
    #     """Display information about the resume template"""
    #     st.subheader("🎨 Resume Template Information")
        
        st.markdown("""
        <div class="info-box">
        📋 <strong>Company Resume Template - Batpharma Style</strong><br/><br/>
        Our resume builder converts uploaded resumes to match our professional company template with the following features:
        </div>
        """, unsafe_allow_html=True)
        ...
        st.info("💡 The template is based on our company standard format (Batpharma-resume.pdf) located in the templates folder.")
        ...
        <strong>📝 For optimal resume conversion:</strong><br/>
        • Use clear, readable fonts in your original resume<br/>
        • Ensure contact information is prominently displayed<br/>
        • List skills and experience in a structured format<br/>
        • Include relevant keywords for your industry<br/>
        • Review and edit extracted information before generating
        </div>
        """, unsafe_allow_html=True)
